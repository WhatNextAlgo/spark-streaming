...
object StreamingJoins {
  ...
  def joinStreamWithStatic() ={
    val streamBandDF = spark.readStream
      .format("socket")
    ...
    // join happen PER Batch
    val streamBandsGuitaristDF = streamBandDF
      .join(guitarPlayers,guitarPlayers.col("band") === streamBandDF.col("id"),"inner")
    ...
    streamBandsGuitaristDF.writeStream
      .format("console")
      .outputMode("append")
      .start()
      .awaitTermination(10000)

    print(streamBandsGuitaristDF.isStreaming)
  }
